...
2. fall back to `Data.TaggedError` only when the error payload is not
   meaningfully serializable or schema-shaped
...
Use `Schema.TaggedErrorClass` by default when the error can be described with
schemas.
...
- the error shape can participate in schema-based tooling and encode/decode
  flows
...
This is also a good default for domain errors when their payload is
schema-friendly.

### 2. `Schema.ErrorClass` for schema-backed errors without `_tag` routing

Use `Schema.ErrorClass` when you want schema-defined error objects but do not
specifically need tag-based pattern matching.
...
When validating external input, Effect's schema APIs return `SchemaError` in the
error channel.
...
For application code, it is often better to convert `SchemaError` into a domain
...
The vendored repo uses schema-backed errors in places like SQL, RPC, sockets,
and HTTP APIs.
...
When an error comes from a library, runtime API, or generic `Error`, prefer
wrapping it in a typed error instead of leaking the foreign error directly
through your domain or protocol boundary.
...
Wrap the foreign error in a schema-backed typed error and preserve the original
error in a `cause` field.

Prefer using:

- `Schema.Defect` when you want to preserve a generic encoded defect
- `Schema.DefectWithStack` when the stack should be preserved in the schema
  contract
...
- a lower-level subsystem failure should be surfaced through a typed domain or
  protocol error
- you need to preserve the underlying failure for debugging without leaking the
  foreign type as the public error contract
...
- callers must understand foreign error shapes instead of your own typed error
  model
...
Use `match` when you want to fully fold the typed error channel into a success
value.
...
Use `orDie` when an error channel should be treated as unrecoverable from this
point onward.
...
Interrupts signal that the fiber should stop. They should not usually be
...
When handling full causes, use `Cause.isInterruptReason`, `Cause.hasInterrupts`,
or filtering over `cause.reasons`.
...
Interrupts often need different operational behavior.
...
- use `Schema.TaggedErrorClass` or `Schema.ErrorClass` when the error contract
  itself matters
...
- leaking `SchemaError` deep into domain code when it should be normalized at
  the boundary
