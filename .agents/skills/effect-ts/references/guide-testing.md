...
This guide is based on the vendored `@effect/vitest` package in
`./.repos/effect`.
...
When testing Effect code with Vitest, prefer `@effect/vitest` over manually
calling `Effect.runPromise`, `Effect.runSync`, or ad hoc runtime setup inside
...
3. If tests need isolated layer instances per test, use multiple separate
   `it.layer(...)` calls.
...
Do not default to `.pipe(Effect.provide(SomeLayer))` inside test bodies when
`layer(...)` or `it.layer(...)` expresses the setup more clearly.
...
Use `it.live` when the test should use live services instead of the default test
environment.
...
The vendored repo also uses `expect` in some tests, but for skill guidance
prefer `assert` in Effect-based tests because it keeps tests more uniform and
explicit inside Effect programs.
...
Use them exactly as you would with Vitest, but return `Effect` from the test
body.

## Property Testing

There are two different property-test entrypoints and they do not have identical
behavior.
...
Unlike top-level `it.prop`, `it.effect.prop` does support `Schema` inputs by
converting them with `Schema.toArbitrary`.
...
This is also the preferred alternative to calling `Effect.provide(...)` inside
each individual test body.
...
- if tests need isolated layer instances per test, use multiple separate
  `it.layer(...)` calls
- use local `Effect.provide(...)` in tests only for true one-off edge cases, not
  as the normal pattern

This matches the general layer guidance: provisioning belongs at the boundary,
and in `@effect/vitest` the test boundary should usually be expressed with
`layer(...)` rather than ad hoc local provisioning.

## `it.layer(...)`

Use `it.layer(...)` inside an existing layered test group to add nested layer
context.

Hard rule:

- use `it.layer(...)` when a specific test or nested test block should get its
  own isolated layered setup
- if isolation matters, prefer multiple separate `it.layer(...)` calls over one
  shared `layer(...)` group
...
- nested layering is meant to extend the current layered test environment, not
  redefine its runtime policy

Use multiple `it.layer(...)` blocks when each test should get its own isolated
layered setup instead of sharing one top-level `layer(...)` group.
...
By default, `layer(...)` merges your layer with the test environment
(`TestClock` and `TestConsole`).

Use `excludeTestServices: true` when you want your layer group to run without
those test-service overrides.

This is useful for tests that should keep live runtime behavior.

### `memoMap`

Use `memoMap` only when you have a specific reason to coordinate layer
memoization manually across test setups.

Most tests should let `@effect/vitest` manage it.

## Scoped Resources In Tests

`@effect/vitest` is designed to work correctly with scoped effects and layered
resources.
...
Avoid manually managing large scopes in test code unless the test specifically
needs that control.
...
Use this only for truly flaky integration-style conditions, not as a substitute
...
Use `makeMethods` when you need to extend or wrap a custom Vitest `it` instance
while preserving Effect-aware helpers.

### `describeWrapped`

Use `describeWrapped` when you want a `describe` wrapper that hands you the
augmented Effect-aware `it` methods directly.
...
Use it only when you have a concrete need to install equality testers for your
Vitest environment.
...
- manually building and tearing down large layer graphs instead of using
  `layer(...)`
