...
This guide is based on the vendored `Schedule` module and its usage across
`./.repos/effect`.
...
A schedule describes when the next step should happen and when execution should
stop.
...
When the timing behavior of an effect matters, prefer expressing it with
`Schedule` rather than ad hoc loops, counters, sleeps, or manual retry
recursion.
...
- `Effect.schedule(schedule)` when you want to delay/reschedule an effect
  directly
...
The tests show that this differs from simple spacing when the action itself
takes time.
...
Use `windowed(duration)` when you want delays to align to the nearest window
boundary.
...
The repo often combines exponential backoff with a more stable spaced fallback
using `Schedule.either(...)`.
...
The module tests verify jittered delays remain within a bounded percentage of
...
Use `andThenResult(left, right)` when one schedule should run to completion and
then another should take over.
...
Use `either` to combine two schedules so both policies influence the resulting
timing.

This appears frequently in repo retry policies that combine exponential growth
with a stable fallback cadence.
...
The OTLP exporter uses this style to honor `retry-after` behavior and otherwise
fall back to a default delay.
...
The module tests also use helpers built on `Schedule.toStepWithSleep(...)` to
inspect schedule behavior precisely.
...
4. Prefer `Schedule.either(...)` or sequencing combinators to compose retry
   phases.
