...
When a project uses Effect, prefer the Effect SQL modules over directly coupling
business code to a native SQL driver API.
...
The business layer should depend on Effect SQL abstractions, not on a raw driver
object.
...
This is the first level of typed SQL usage and is already better than untyped
row access.
...
Prefer integrating SQL with Schema whenever the row shape matters or the query
result crosses a meaningful boundary.
...
- or a SQL helper such as `SqlResolver`, `SqlSchema`, or `SqlModel` when
  appropriate
...
`SqlResolver` is one of the clearest examples of SQL and Schema integration in
the vendored repo.
...
Use `SqlSchema` and `SqlModel` when you want tighter schema integration with SQL
itself.
...
- it is easier to drift away from typed SQL errors, SQL tracing, and reusable
  query helpers

Prefer a layer that provides `SqlClient`, and let domain services depend on
that.
...
- the SQL client implementation can be provided separately from the business
  service
...
Avoid exposing one exported accessor function per SQL service method if it only
...
- or export a real business operation that adds behavior beyond simple
  forwarding

## SQL Resolvers

Use `SqlResolver` when request-style batching or schema-validated
request/response handling is useful.
...
This is one of the strongest typed-query patterns in the repo and should be
preferred over ad hoc batched row mapping when the query fits the resolver
model.
...
Avoid overusing manual `type Row = { ... }` plus custom conversion if the
schema-aware modules already express the shape clearly.

Preferred order for query typing:

1. schema-aware SQL module such as `SqlResolver`, `SqlSchema`, or `SqlModel`
   when it fits
2. typed query literals plus Schema decoding when the query is local
3. only use manual row mapping when the first two options are clearly heavier
   than the problem
...
The runtime-specific SQL migrator packages expose `fromRecord(...)` to define
migrations from an ordered record.
...
Use the runtime-specific `run(...)` helper when you want a startup effect that
runs migrations explicitly.
...
Use the runtime-specific `layer(...)` helper when migrations should run as part
of top-level infrastructure setup.
...
From the vendored packages, this is implemented as
...
If the same migrated SQL environment is reused in tests, create it once and
reuse the layer value.

### Migration Best Practices

- use stable numeric migration IDs
- keep migration files ordered and unique
- do not hand-roll a separate migrations subsystem if `Migrator` already fits
  the project
- keep migration execution at startup or a dedicated operational boundary
- do not bury migration execution inside arbitrary service construction unless
  startup is explicitly the right place
...
- migrated environment layer: merge the SQL client layer with the migration
  layer provided by that client layer
...
- SQL layer or repository works with `SqlError` and schema decode failures where
  appropriate
...
- add explicit spans only where business-level detail matters beyond the
  built-in SQL spans
...
4. top-level application layer composes the database layer with the business
   layers
...
- embedding a native driver directly in business services when Effect SQL
...
- creating ad hoc migration effects without a stable loader shape when
  `fromRecord(...)` already fits the project
- scattering migration execution across multiple subsystems instead of one
