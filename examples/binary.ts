...
const Message = Binary.codec(
  message,
  Schema.Struct({
    header: Schema.Struct({ version: Schema.Number, priority: Schema.Number }),
...
const main = Effect.gen(function* () {
  ...
  yield* Console.log(`print ${JSON.stringify(value)}\n  →  ${hex(printed)}`)
  ...
  yield* Console.log(`codec decode → ${JSON.stringify(decoded)}`)
