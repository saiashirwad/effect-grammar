...
describe("streamElements", () => {
  it("emits one value per element across chunk boundaries", () => {
    ...
    const values = Effect.runSync(
      Grammar.streamElements(chunks, line).pipe(Stream.runCollect),
    );
  ...
  it("dies when an element succeeds without consuming input", () => {
    ...
    assert.throws(
      () =>
        Effect.runSync(
          Grammar.streamElements(chunks, zeroWidth).pipe(Stream.runCollect),
        ),
      (err: unknown) =>
...
describe("parseStream", () => {
  ...
  it("surfaces UpstreamError when the chunk source fails", () => {
    ...
    const r = Effect.runSync(
      Effect.result(parseStream(boom, char("a"))),
    );
  ...
  it("fails with ParseError when the grammar does not match", () => {
    ...
    const r = Effect.runSync(
      Effect.result(Grammar.parseStream(chunks, Grammar.literal("yes"))),
    );
    assert.equal(r._tag, "Failure");
    if (r._tag === "Failure") {
